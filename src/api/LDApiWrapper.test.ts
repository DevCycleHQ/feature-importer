...
describe("LDApiWrapper", () => {
  ...
  describe("Pagination", () => {
    ...
    test("should merge environment data when same flag appears across environment chunks", async () => {
      // Setup: 4 environments (will be split into 2 chunks: [env1, env2, env3], [env4])
      fetchMock.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      // Chunk 1 response: flag with env1, env2, env3 data
      fetchMock.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      // Chunk 2 response: same flag with env4 data
      fetchMock.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
